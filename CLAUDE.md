...
**Avaliação assíncrona (estilo Uber/iFood) e mensagem do gestor ao resolver (extensão do item 8, ainda não implementada):**
- **Sem notificação push/e-mail** — fora de escopo (ver "Notificações em tempo real" em "Fora de escopo"), não introduzir serviço de e-mail só pra isso. Em vez disso, gatilho passivo no login: ao abrir o **Mapa** autenticado, se o usuário tiver algum `Problem` próprio `RESOLVIDO` sem `resolutionRating`, um modal pede a avaliação (estrelas 1–5) assim que a tela carrega — igual ao padrão de apps tipo Uber/iFood pedindo nota do último pedido/corrida ao abrir o app. Critério único cobre os dois casos — gestor resolveu (nunca pede nota na hora) e autor resolveu sem informar nota na hora (já era opcional) — sem duplicar regra. Modal tem opção "Agora não" (fecha sem salvar; reaparece no próximo login enquanto não for avaliado — não implementar dispensa permanente, é trade-off aceito de escopo de protótipo).
- Endpoint novo pra listar pendências (ex.: `GET /problems/pendentes-avaliacao`, autenticado, retorna os `Problem` do usuário logado com essas condições) e outro pra registrar a nota (ex.: `PATCH /problems/:id/avaliar`) — só o autor original pode chamar, só se `status = RESOLVIDO` e `resolutionRating` ainda for nulo.
- **Mensagem opcional do gestor ao resolver:** campo novo `Problem.resolutionNote` (String, opcional). Preenchido só quando quem resolve é gestor (textarea opcional no fluxo de resolução do `PainelGestor`); exibido na página de detalhe do problema pra qualquer visitante, junto com o status.
